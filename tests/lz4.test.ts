import { decompressLz4Block, decompressLz4Sequence, isLz4Frame, lz4Sequences, parseLz4FrameHeader } from '../src/utils/lz4';

const FLG_DEFAULT = 0x68; // version 1, block independence, content size

function buildFrame (contentSize: number, blockDataSizes: number[], flg = FLG_DEFAULT, blockSizeCode = 4) {
...
describe('parseLz4FrameHeader', () => {
  ...
  test('rejects a frame with block checksums', () => {
    expect(() => parseLz4FrameHeader(buildFrame(100, [10], FLG_DEFAULT | 0x10))).toThrow('block checksum');
  });
  ...
  test('rejects an invalid block max size code', () => {
    expect(() => parseLz4FrameHeader(buildFrame(100, [10], FLG_DEFAULT, 2))).toThrow('block max size');
  });
});
...
describe('decompressLz4Block', () => {
  ...
  test('decompresses a block with a match', () => {
    ...
    expect(Array.from(decompressLz4Block(block, 64)))
      .toEqual([0x61, 0x62, 0x63, 0x64, 0x61, 0x62, 0x63, 0x64]);
  });
